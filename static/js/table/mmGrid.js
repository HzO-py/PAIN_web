...
!function($){
    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el){
            ...
            if(opts.checkCol){
                var chkHtml = opts.multiSelect ?  '<span style="display: inline-block;\n' +
                    '*display: inline;*zoom:1;height:2em;font-weight:bold;line-height:2;vertical-align: top;">本页全选&nbsp;</span><input type="checkbox" class="checkAll" >'
                    : '<input type="checkbox" disabled="disabled" class="checkAll">';
                opts.cols.unshift({title:chkHtml,width: 120, align: 'center' , sortable: false, lockWidth: true, checkCol: true, renderer:function(){
        ...
        , _populate: function(items){
            ...
            if(items && items.length !== 0 && opts.cols){
                ...
                for(var rowIndex=0; rowIndex < items.length; rowIndex++){
                    ...
                    // $.data($trs.eq(rowIndex)[0],items[rowIndex]);
                }
            }else{
                this._insertEmptyRow();
                this._showNoData();
        ...
        , select: function(args){
            ...
            if(typeof args === 'number'){
                ...
                if(!$tr.hasClass('selected')){
                    ...
                    selectList.push($tr.find('.mmg'+this._id+'-col3')[0].innerText);
                    localStorage.setItem('selectList',JSON.stringify(selectList));

                    var allselectList = JSON.parse(localStorage.getItem('allselectList'));
                    allselectList.push($tr.find('.mmg'+this._id+'-col4')[0].innerText);
                ...
                var selectList = [];
                var allselectList = [];
                var allfind = $body.find('tr > .mmg'+this._id+'-col3');
                var allfindall = $body.find('tr > .mmg'+this._id+'-col4');
                for(var i in allfind){
                    if(isNaN(i))
                        break;
                    var el = allfind[i].innerText;
                    selectList.push(el);
                }
                for(var i in allfindall){
                    if(isNaN(i))
                        break;
                    var el = allfindall[i].innerText;
                    allselectList.push(el);
                }
                localStorage.setItem('selectList',JSON.stringify(selectList));
                localStorage.setItem('allselectList',JSON.stringify(allselectList));
        ...
        , deselect: function(args){
            ...
            if(typeof args === 'number'){
                ...
                var selectList = JSON.parse(localStorage.getItem('selectList'));
                var allselectList = JSON.parse(localStorage.getItem('allselectList'));
                selectList.remove($tr.find('.mmg'+this._id+'-col3')[0].innerText);
                allselectList.remove($tr.find('.mmg'+this._id+'-col4')[0].innerText);
                localStorage.setItem('selectList',JSON.stringify(selectList))
                localStorage.setItem('allselectList',JSON.stringify(allselectList))
                if(opts.checkCol){
