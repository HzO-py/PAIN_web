...
$(function(){
    ...
    v = getCookieValue("remained");//获取cookie值
    if(v>0){
...
function sendCode(obj){
    ...
    if(result){
        doPostBack('/verifycode/',backFunc1,{"phoneNum":phoneNum,"sendtype":"reset"});
        addCookie("remained",10,10);//添加cookie记录,有效时间10s
        settime(obj);//开始倒计时
    }
}
...
function doPostBack(url,backFunc,queryParam) {
    $.ajax({
        ...
        url : url,// 请求的action路径
        data:queryParam,
        error : function(response) {// 请求失败处理函数
        },
