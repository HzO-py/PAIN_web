            ...
            $liEle.on("mouseover", function () {
                ...
                if ($tipEle) {
                    if (obj.options.tips === "default") {
                        ...
                        $('#desc')[0].innerText=obj.options.desc[len-1]
                        $('#descp')[0].style.display = 'block';
                        $('#descp')[0].style.left = this.offsetLeft+'px';
                        $('#descp')[0].style.top = this.offsetTop+30+'px';
                    }
                }
            });
            $liEle.on("mouseout", function () {
                $('#desc')[0].innerText='此处将显示对应评分参考';
                $('#descp')[0].style.display = 'none';
                if (!obj.options.isReScore && obj.isScoreFinish) {
                    return;
                }
